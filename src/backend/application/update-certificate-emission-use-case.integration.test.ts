...
describe('UpdateCertificateEmissionUseCase (Integration)', () => {
    describe('atualização de nome', () => {
        it('deve atualizar o nome da emissão no banco', async () => {
            ...
            await prisma.certificateEmission.create({
                data: { id: '1', title: 'Nome Original', user_id: '1', status: CERTIFICATE_STATUS.DRAFT },
            })
            ...
            const record = await prisma.certificateEmission.findUnique({ where: { id: '1' } })
    ...
    describe('atualização de mapeamento de variáveis', () => {
        ...
        it('deve reverter o reset de status das linhas quando falha ao atualizar certificado na transação', async () => {
            ...
            await expect(
                useCase.execute({ id: '1', userId: '1', variableColumnMapping: { nome: 'nome' } }),
            ).rejects.toThrow()

            const rows = await prisma.dataSourceRow.findMany({ where: { data_source_id: '1' } })
            expect(rows.every(r => r.processing_status === 'RUNNING')).toBe(true)
        })
    })
})