...
describe('DeleteCertificateEmissionUseCase (Integration)', () => {
    it('deve excluir a emissão e todas as suas relações do banco', async () => {
                    ...
                    create: {
                        ...
                        DataSourceRow: {
                            create: [{
                                ...
                                source_row_index: 1,
                                DataSourceValue: {
                                    create: [{ column_name: 'nome', value: 'Alice' }],
                                },
                            }],
                        },
                    },
        ...
        expect(await prisma.certificateEmission.findUnique({ where: { id: '1' } })).toBeNull()
        expect(await prisma.template.findFirst({ where: { certificate_emission_id: '1' } })).toBeNull()
        expect(await prisma.templateVariable.findMany({ where: { template_id: '1' } })).toHaveLength(0)
        expect(await prisma.dataSource.findFirst({ where: { certificate_emission_id: '1' } })).toBeNull()
        expect(await prisma.dataSourceFile.findMany({ where: { data_source_id: '1' } })).toHaveLength(0)
        expect(await prisma.dataSourceColumn.findMany({ where: { data_source_id: '1' } })).toHaveLength(0)
        expect(await prisma.dataSourceRow.findMany({ where: { data_source_id: '1' } })).toHaveLength(0)
        expect(await prisma.dataSourceValue.findMany({ where: { data_source_id: '1' } })).toHaveLength(0)
        expect(await prisma.email.findFirst({ where: { certificate_emission_id: '1' } })).toBeNull()
    })
})