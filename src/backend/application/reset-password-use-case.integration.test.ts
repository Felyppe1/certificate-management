...
describe('ResetPasswordUseCase (Integration)', () => {
    it('deve atualizar a senha e limpar o código de redefinição no banco', async () => {
        ...
        await makeUseCase().execute({ email: EMAIL, code: CODE, newPassword: NOVA_SENHA })

        const user = await prisma.user.findUnique({ where: { id: 'user-1' } })
        expect(user!.password_hash).not.toBe('hash-antigo')
        const senhaValida = await bcrypt.compare(NOVA_SENHA, user!.password_hash!)
        expect(senhaValida).toBe(true)
        expect(user).toMatchObject({ reset_password_code: null, reset_password_expires_at: null })
    })

    it('deve lançar erro quando e-mail não existe', async () => {
        await expect(
            makeUseCase().execute({ email: 'inexistente@test.com', code: CODE, newPassword: NOVA_SENHA }),
    ...
    it('deve lançar erro quando código está expirado', async () => {
        ...
        await expect(
            makeUseCase().execute({ email: EMAIL, code: CODE, newPassword: NOVA_SENHA }),
    ...
    it('deve lançar erro quando código está incorreto', async () => {
        ...
        await expect(
            makeUseCase().execute({ email: EMAIL, code: '000000', newPassword: NOVA_SENHA }),
