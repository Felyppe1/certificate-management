import { ICertificatesRepository } from '@/backend/application/interfaces/repository/write/icertificates-repository'
import {
    CertificateEmission,
    CERTIFICATE_STATUS,
    INPUT_METHOD,
} from '@/backend/domain/certificate'
import { COLUMN_TYPE } from '@/backend/infrastructure/repository/prisma/client/client'
...
export class PrismaCertificatesRepository
    extends PrismaRepository
    implements ICertificatesRepository
{
    ...
    async delete(id: string): Promise<void> {
        await this.prisma.certificateEmission.delete({
            where: { id },
        })
    }

    async markAsGeneratedIfNotAlready(id: string): Promise<void> {
        await this.prisma.certificateEmission.updateMany({
            where: {
                id,
                status: {
