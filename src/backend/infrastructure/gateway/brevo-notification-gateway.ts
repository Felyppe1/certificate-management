...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Redefinição de Senha - Certifica</title>
    <style>
        body { margin: 0; padding: 0; background-color: #0d1117; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased; }
        table { border-spacing: 0; border-collapse: collapse; }
        td { padding: 0; }
        ...
        <table class="main" width="100%">
            ...
            <tr>
                <td class="content">
                    ...
                    <p>Use o código abaixo para redefinir sua senha na plataforma <strong>Certifica</strong>.</p>
                    <div class="code-box">
                        <div class="code">{{CODE}}</div>
                    </div>
                    <p class="note">Este código expira em 15 minutos. Se você não solicitou isso, pode ignorar este e-mail.</p>
...
export class BrevoNotificationGateway implements INotificationGateway {
    ...
    async sendEmailVerification(email: string, code: string): Promise<void> {
        ...
        await this.send(email, 'Verifique seu e-mail no Certifica', html)
    }
    ...
    async sendResetPasswordCode(email: string, code: string): Promise<void> {
        ...
        await this.send(email, 'Redefinição de senha no Certifica', html)
    }
    ...
    async sendAccessRequest(email: string): Promise<void> {
        ...
        await this.send(
            env.OWNER_EMAIL,
            'Solicitação de acesso ao Certifica',
            html,
        )
    }

    async sendAccessGranted(email: string): Promise<void> {
        const html = accessGrantedEmailTemplate.replaceAll(
            '{{APP_URL}}',
            env.NEXT_PUBLIC_BASE_URL,
        )

        await this.send(email, 'Acesso ao Certifica Concedido', html)
    }
}
