import { ExternalUserAccount, ExternalUserAccountsRepository, Provider } from "@/backend/application/interfaces/external-user-account";
import { prisma } from ".";

export class PrismaExternalUserAccountsRepository implements ExternalUserAccountsRepository {
    ...
    async save(account: ExternalUserAccount) {
        await prisma.externalUserAccount.create({
            data: {
                ...
                refresh_token_expiry_datetime: account.refreshTokenExpiryDateTime,
            }
        })
    }
