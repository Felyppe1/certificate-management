...
export async function GET(request: Request) {
    ...
    if (!code) {
        ...
        return NextResponse.redirect(process.env.NEXT_PUBLIC_BASE_URL + '/entrar')
    }
    ...
    const externalUserAccountsRepository = new PrismaExternalUserAccountsRepository()
    ...
    if (payload?.email) {
        ...
        const externalAccount = await externalUserAccountsRepository.getById(user.id, 'GOOGLE')
